...
class DotAudio {
  ...
  update() {
	...
	DotAudio.removeSynth(this);
      ...
      let note = 68-Math.round(68*Math.log(this.dot.weight)/Math.log(200));
      let f = 55*Math.pow(2, note/12);
      if (f<27.5) f = 27.5;
      let ff = f*(1+1.4*Math.pow(this.dot.weight, .01)*Math.sqrt(this.dot.f[0]*this.dot.f[0] + this.dot.f[1]*this.dot.f[1]));
      if (ff<f) ff = f;
